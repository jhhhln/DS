...
#含义是给定常规渠道和加急渠道的inventory position之后如何订货和每条路径的成本
    ...
    def benchmark_DI_policy(self,demand,sample,x_init=None,q_init=None,inventory_level=0):
        ...
        for delta in delta_range:
            ...
            #计算最优的Se
                def find_best_Se(self, overshoot_record, h=None, p=None):
        h = self.holding_cost if h is None else h
        p = self.stockout_cost if p is None else p
        num_of_iter = overshoot_record.shape[0]
        best_quantile_se = np.quantile(np.repeat([self.cumsum_sample[self.lead_time_express, -100:]], num_of_iter, axis=0)
                                       - overshoot_record[:, -100:], p/(p+h))
        return np.maximum(best_quantile_se, 0)
            cost=record['average_total_cost']
            DI_cost_record.append(cost)
        min_cost_idx = np.argmin(DI_cost_record)
        optimal_delta = delta_range[min_cost_idx]
