    ...
    def DDI_policy(self,demand,Se=None,D_2_constraint=False,inventory_level=0):
        ...
        for t in range(period_length):
            ...
            if order_record_expe.shape[1] < period_length:
                ...
                if D_2_constraint:
                    ###
                    IP_e=inv_level_record[:,[t]]+order_record_expe[:,t:t+self.l_e+1].sum(axis=1)[:,None]+order_record_regular[:,t:t+self.l_e].sum(axis=1)[:,None]
    ...
    def SDI_policy(self,demand,inventory_level=0):
        ...
        for t in range(period_length):
            if order_record_expe.shape[1] < period_length:
                ...
                +order_record_regular[:,t:t+self.l_e].sum(axis=1)[:,None]
                order_e=np.maximum(np.ones(IP_e.shape) * self.Se+order_record_regular[:,t+self.l_e] - IP_e, 0)
                order_record_expe=np.hstack((order_record_expe,order_e))
    ...
    def cal_order_up_to(self,demand,S_e,S_r,x_init,q_init,inventory_level=0,constraint_D1=False):
        ...
        for t in range(period_length):
            if order_record_expe.shape[1] < period_length:
                ...
                order_record_expe=np.hstack((order_record_expe,order_e))
                overshoot=np.maximum(IP_e+order_record_regular[:,[t+self.l_e]]-np.ones(IP_e.shape) * S_e, 0)
    ...
    def cal_order_up_to_with_r(self,demand,S_e,r,x_init,q_init,inventory_level=0,constraint_D1=False):
        ...
        for t in range(period_length):
            if order_record_expe.shape[1] < period_length:
                ...
                IP_e=inv_level_record[:,[t]]+order_record_expe[:,t:t+self.l_e].sum(axis=1)[:,None]+order_record_regular[:,t:t+self.l_e].sum(axis=1)[:,None]
                ###
                order_e=np.maximum(np.ones(IP_e.shape) * S_e+order_record_regular[:,t+self.l_e] - IP_e, 0)
                order_record_expe=np.hstack((order_record_expe,order_e))
                overshoot=np.maximum(IP_e+order_record_regular[:,t+self.l-np.ones(IP_e.shape) * S_e, 0)
                overshoot_record=np.hstack((overshoot_record,overshoot))
            if order_record_regular.shape[1] < period_length:
                # 确保索引不越界
                end_idx = min(t + self.l_r, order_record_regular.shape[1])
