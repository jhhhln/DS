...
#TBS策略中加急渠道是order_up_to,常规渠道是r
    ...
    def cal_order_up_to_with_r_constraint(self,demand,S_e,r,x_init,q_init,inventory_level=0):
        ...
        order_record_expe=np.zeros((iter_num,period_length))
    ...
    def TBS_policy_constraint(self, sample, demand, mean, x_init=None, q_init=None):   
        x_init = self.x_init_DDI if x_init is None else x_init
        q_init = self.q_init if q_init is None else q_init
    ...
    def TBS_policy(self, sample, demand, mean, x_init=None, q_init=None):
        ...
        for r in r_range:
            ...
            #对于参数对(r,Se)考虑服务水平
            result = self.cal_order_up_to_with_r_constraint(self,demand,best_Se,r,x_init,q_init,inventory_level=0)
            cost = result['average_total_cost']
...
print(f"最优 r = {best_r:.4f}, 对应的最优 S_e = {best_Se:.4f}, 最小平均成本 = {TBS_cost_record[best_index]:.4f}")
