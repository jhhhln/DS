...
#TBS策略中加急渠道是order_up_to,常规渠道是r
    ...
    def cal_fill_rate(self, demand, result_record_dict, policy_name, policy_para,
        Se=0, measure_type='regular'):
        """
        计算库存 fill rate，考虑所有模拟路径
        """
        ...
        for t in range(self.period_length - self.lead_time_regular):
            ...
            for i in range(num_of_iter):
                current_pipeline_r = order_record_r[i, t : t + self.lead_time_regular + 1].reshape((1, -1))
                current_pipeline_e = order_record_e[i, t : t + self.lead_time_express + 1].reshape((1, -1))
                inventory_level = inv_level_record[i, t]

                # Monte Carlo：未来需求用所有 demand 路径
                ...
                if policy_name == 'DDI':
                    sample_result = self.measure_DDI_policy(
                        demand=current_future_demand_path,
                        x_init=x_init, q_init=q_init,
                        inventory_level=inventory_level,
                        S=policy_para[0],
                        measure_constraint=False,
                        system_type='unconstrained')
                else:
                    sample_result = self.measure_policy_cost(
                        demand=current_future_demand_path,
                        x_init=x_init, q_init=q_init,
                        inventory_level=inventory_level,
                        Se=Se,
                        policy_name=policy_name, policy_para=policy_para,
                        measure_constraint=False,
                        system_type='unconstrained'
                    )

                Sr_all.append((sample_result['inv_level_record'][:, -1] > 0).mean())
                Se_all.append((sample_result['inv_level_record'][:, self.lead_time_express + 1] > 0).mean())

            # 平均所有路径
            service_level_Sr.append(np.mean(Sr_all))
            service_level_Se.append(np.mean(Se_all))
