...
#TBS策略中加急渠道是order_up_to,常规渠道是r
    ...
    def cal_fill_rate(self, demand, result_record_dict,policy_name):
        #对于每条路径的每个节点，计算在到达时刻的总库存是否能满足需求
        #相当于可以得到一个[N,T]数组，每一个点都有若干个0-1变量表达是否满足(对应着若干条路径)，然后对每个点都可以得到一个概率，最后对每一列
        ...
        for t in range(period_length - self.l_r):
            ...
            for sample in range(num_of_iter):
                ...
                if policy_name == 'DDI':
                    sample_result=self.single_lost_sales(demand=current_future_demand_path, S=None, inventory_level=current_inventory_level)
