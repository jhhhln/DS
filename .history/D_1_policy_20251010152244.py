...
#含义是给定常规渠道和加急渠道的inventory position之后如何订货和每条路径的成本
    ...
    def DI_policy(self,demand,sample,x_init=None,q_init=None,inventory_level=0):
        ...
        delta_range = np.arange(0, 2*self.Sr, (2*self.Sr)/self.num_search_range)

        DI_cost_record =[]
        for delta in delta_range:
            record=self.cal_order_up_to(sample,self.Se,delta+self.Se,x_init,q_init,inventory_level=0,D_2_constraint=True)
